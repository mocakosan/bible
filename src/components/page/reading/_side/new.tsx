...
function NewTestament({ readState, menuIndex }: Props) {
    ...
    const getChapterStyle = useCallback((book: number, chapter: number) => {
        ...
        try {
            ...
            const status = getChapterStatus(book, chapter);
    ...
    const RenderItems = useCallback(
                ...
                <View key={`${book}-${refreshKey}`}>
                    ...
                    >
                        {Array.from({ length }).map((_, index) => {
                            ...
                            // 디버깅용 로그 (마태복음 1장만)
                            if (book === 40 && chapter === 1) {
                                console.log(`New RenderItems: 마태복음 1장 - isRead: ${isRead}, status: ${status}`);
                            }

                            const styleProps = getChapterStyle(book, chapter);
        ...
        [getChapterStyle, isChapterReadSync, getChapterStatus, planData, onNavigate, refreshKey, readState]
    );
